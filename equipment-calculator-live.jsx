  ...
  return (
    <div className="flex flex-col gap-1.5 shrink-0">
      <button
        ...
        disabled={disabled}
        className={
          "w-9 h-9 rounded-full text-white text-lg font-black flex items-center justify-center transition-transform shadow-md " +
          (disabled
            ? "opacity-40 cursor-not-allowed"
            : "active:scale-90 active:shadow-none active:translate-y-1")
        }
...
function Badge({ isInput }) {
  return (
    <span
      className={
        "text-sm font-black px-2.5 py-1 rounded-full tracking-wide " +
        (isInput ? "bg-white/80 text-slate-700" : "bg-black/10 text-slate-600")
      }
    >
    ...
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-black text-base" style={{ color: palette.text }}>
              {label}
            </div>
            {sub && <div className="text-sm text-slate-600 font-bold">{sub}</div>}
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 mt-3">
        <div
          className="flex-1 rounded-2xl px-3 py-2 text-center font-mono font-black text-2xl sm:text-2xl text-white shadow-inner"
    ...
    <div className="inline-flex rounded-full p-1 gap-1" style={{ backgroundColor: "#fff" }}>
      {options.map((opt) => (
        <button
          ...
          className={
            "px-3 py-1 rounded-full text-sm font-black transition-all " +
    ...
    >
      <div className="max-w-4xl mx-auto px-4 py-7 sm:px-8 sm:py-10">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur rounded-full px-5 py-2 mb-3">
            <span className="text-yellow-300 text-xl leading-none">🎉</span>
            <span className="text-white text-base sm:text-lg font-black tracking-wide">
              장비가 · 임대료 한번에 계산!
            </span>
          </div>
          <h1 className="text-white text-3xl sm:text-4xl font-black drop-shadow-md">
            🏗️ 적정가 환산기
          </h1>
          <p className="text-white/95 text-base sm:text-base font-bold mt-2">
            둘 중 하나를 고정하면, 다른 하나를 계산해드려요
          </p>
          <div className="flex justify-center gap-2 mt-4">
            <button
              onClick={loadExample}
              className="bg-yellow-300 text-purple-900 font-black text-sm px-4 py-2 rounded-full shadow-lg active:shadow-none active:translate-y-1 transition-all flex items-center gap-1.5"
            >
              <span className="text-base leading-none">✨</span> 예시값 불러오기
            </button>
            <button
              onClick={reset}
              className="bg-white/90 text-slate-700 font-black text-sm px-4 py-2 rounded-full shadow-lg active:shadow-none active:translate-y-1 transition-all flex items-center gap-1.5"
            >
              <span className="text-base leading-none">🔄</span> 초기화
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <button
            ...
            className={
              ...
              (mode === "price" ? "bg-white border-yellow-300 scale-105" : "bg-white/70 border-transparent")
            }
          >
            <div className="text-sm font-black text-orange-500 mb-0.5">기능 ①</div>
            <div className="font-black text-slate-800 text-sm">🏗️ 장비가격 고정 → 💰 임대료 산정</div>
          </button>
          <button
            onClick={() => setMode("rent")}
            className={
              "rounded-2xl p-3.5 text-left border-4 transition-all " +
              (mode === "rent" ? "bg-white border-yellow-300 scale-105" : "bg-white/70 border-transparent")
            }
          >
            <div className="text-sm font-black text-orange-500 mb-0.5">기능 ②</div>
            <div className="font-black text-slate-800 text-sm">💰 임대료 고정 → 🏗️ 장비가격 산정</div>
          </button>
        </div>

        {/* Dials */}
        <div className="bg-white/10 backdrop-blur rounded-3xl p-4 sm:p-5 mb-5">
          <div className="text-sm font-black text-white/90 tracking-wide uppercase mb-2 pl-1">
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <div className="font-black text-base" style={{ color: COLORS.price.text }}>
                    장비가격
                  </div>
                </div>
              ...
              {priceCurrency === "KRW" ? (
                ...
                <>
                  <div className="flex items-center gap-3 mt-3">
                    <div
                      className="flex-1 rounded-2xl px-3 py-2 text-center font-mono font-black text-xl sm:text-2xl text-white shadow-inner"
                      style={{ backgroundColor: COLORS.price.text }}
                    >
              ...
              <p className="text-sm font-bold mt-2" style={{ color: COLORS.price.text }}>
        ...
        <div className="bg-gradient-to-br from-yellow-300 via-amber-300 to-orange-300 rounded-3xl p-5 sm:p-6 border-4 border-white shadow-xl mb-5">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-orange-700 text-lg leading-none">🧭</span>
            <span className="text-orange-800 font-black text-sm tracking-wide uppercase">
          ...
          <div className="text-center">
            ...
            <div className="text-orange-800/80 text-sm font-bold mt-1">
              {mode === "price"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-5">
            <div className="bg-white/70 rounded-2xl p-3 text-center">
              <span className="block text-orange-600 mb-1 text-sm leading-none">📊</span>
              <div className="text-sm font-bold text-orange-700">총 임대수익률(연)</div>
              <div className="font-mono font-black text-base text-orange-900">{fmtPct(grossAnnualYield, 1)}</div>
            </div>
            <div className="bg-white/70 rounded-2xl p-3 text-center">
              <span className="block text-orange-600 mb-1 text-sm leading-none">🔧</span>
              <div className="text-sm font-bold text-orange-700">순수익률(비용차감후)</div>
              <div
            ...
            <div className="bg-white/70 rounded-2xl p-3 text-center">
              <span className="block text-orange-600 mb-1 text-sm leading-none">🚚</span>
              <div className="text-sm font-bold text-orange-700">투자 회수기간</div>
              <div className="font-mono font-black text-base text-orange-900">
                {isFinite(paybackMonths) ? paybackMonths.toFixed(1) + "개월" : "—"}
              </div>
            </div>
            <div className="bg-white/70 rounded-2xl p-3 text-center">
              <span className="block text-orange-600 mb-1 text-sm leading-none">🏛️</span>
              <div className="text-sm font-bold text-orange-700">금리 스프레드</div>
              <div
                className="font-mono font-black text-base"
          ...
          <p className="text-orange-800/70 text-xs font-semibold text-center mt-2">
            순수익률 = 총 임대수익률({fmtPct(grossAnnualYield, 1)}) − 정비·운송·보험비({fmtPct(costPct, 1)}) · 회수기간·스프레드는 순수익률 기준
          </p>

          <div
            className="mt-4 rounded-2xl p-3.5 text-sm font-bold leading-relaxed text-center"
            style={{
        ...
        <div className="bg-white rounded-3xl p-5 sm:p-6 border-4 border-emerald-300">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-2xl">📉</span>
            <div>
              <div className="font-black text-emerald-700 text-base">3년 보유 후 되팔면?</div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
            <div className="bg-emerald-50 rounded-2xl p-3 text-center">
              <div className="text-sm font-bold text-emerald-700 mb-1">3년 누적 임대료</div>
              <div className="font-mono font-black text-sm text-emerald-900">{fmtEok(threeYr.rentTotal)}</div>
            </div>
            <div className="bg-red-50 rounded-2xl p-3 text-center">
              <div className="text-sm font-bold text-red-600 mb-1">3년 정비·운송·보험비</div>
              <div className="font-mono font-black text-sm text-red-700">−{fmtEok(threeYr.opCostTotal)}</div>
            </div>
            <div className="bg-emerald-50 rounded-2xl p-3 text-center">
              <div className="text-sm font-bold text-emerald-700 mb-1">3년 후 중고값</div>
              <div className="font-mono font-black text-sm text-emerald-900">{fmtEok(threeYr.resale)}</div>
            </div>
            <div className="bg-emerald-50 rounded-2xl p-3 text-center">
              <div className="text-sm font-bold text-emerald-700 mb-1">3년 순수익</div>
              <div
            ...
            <div className="bg-emerald-50 rounded-2xl p-3 text-center">
              <div className="text-sm font-bold text-emerald-700 mb-1">연환산 총수익률</div>
              <div
                className="font-mono font-black text-sm"
          ...
          <p className="text-sm text-slate-500 font-bold mt-3 text-center">
            3년 순수익 = (월 임대료 × 가동월수 {opMonths}개월 × 3년) − (장비가격 × 정비등비용 {fmtPct(costPct, 1)} × 3년) + 3년 후 중고값 − 최초 장비가격
          </p>
        </div>

        <p className="text-center text-white/90 text-sm font-bold mt-5">
